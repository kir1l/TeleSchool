"""
Модуль `settings_menu` отвечает за функциональность меню настроек бота.

Модуль `settings_menu` предоставляет следующую функциональность:

- `open_settings`: Отображает меню настроек с изображением настроек и клавиатурой настроек.
- `settings_changeName`: Обрабатывает процесс изменения имени пользователя. Устанавливает состояние в `SettingsStates.change_name`, отображает изображение с подписью, предлагая пользователю ввести новое имя, и обновляет имя пользователя в базе данных.
- `settings_changeSex`: Обрабатывает процесс изменения пола пользователя. Устанавливает состояние в `SettingsStates.change_sex`, отображает изображение с подписью, предлагая пользователю выбрать новый пол, и обновляет пол пользователя в базе данных.
- `settings_changeSkin`: Обрабатывает процесс изменения цвета кожи пользователя. Устанавливает состояние в `SettingsStates.change_skinColor`, отображает изображение с подписью, предлагая пользователю выбрать новый цвет кожи, и обновляет цвет кожи пользователя в базе данных.
"""
