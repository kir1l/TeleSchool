"""
Функция регистрации - это обработчик запросов обратного вызова, который срабатывает, когда пользователь нажимает кнопку "начать регистрацию". Она проверяет, зарегистрирован ли пользователь уже, и если нет, устанавливает состояние в Reg_form.name и отправляет сообщение с фотографией, предлагая пользователю ввести имя.

Функция регистрации имени - это обработчик сообщений, который срабатывает, когда пользователь вводит свое имя. Она сохраняет имя в состоянии и устанавливает состояние в Reg_form.sex, затем отправляет сообщение с фотографией, предлагая пользователю выбрать пол.

Функция регистрации пола - это обработчик запросов обратного вызова, который срабатывает, когда пользователь выбирает пол. Она сохраняет пол в состоянии и устанавливает состояние в Reg_form.skinColor, затем отправляет сообщение с фотографией, предлагая пользователю выбрать цвет кожи.

Функция завершения регистрации - это обработчик запросов обратного вызова, который срабатывает, когда пользователь выбирает цвет кожи. Она сохраняет цвет кожи в состоянии, создает новый экземпляр Player с информацией пользователя, сохраняет данные в базу данных и кэш, а также выдает пользователю стартовый баланс в размере $5000. Затем она устанавливает глобальную переменную registration_state в False и отправляет сообщение с фотографией, подтверждающее завершение регистрации.

Функция некорректного выбора пола - это обработчик сообщений, который срабатывает, когда пользователь пытается вручную ввести свой пол или цвет кожи вместо использования предоставленных кнопок. Она отправляет сообщение, в котором просит пользователя использовать кнопки.
"""
